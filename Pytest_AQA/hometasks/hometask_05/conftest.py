...
def scope_function(api_data_function=10):
...
def scope_class(api_data_class=10):
...
def scope_module(api_data_module=10):
...
def scope_package(api_data_package=10):
...
def scope_session(api_data_session=10):
...
# Задача 3
# Перепишіть тест з попереднього уроку де в нас був Class TestA
# та містив setup_method та teardown_method зробіть три теста в середині цього класу.
